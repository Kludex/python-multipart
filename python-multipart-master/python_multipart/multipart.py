...
@@ -215,6 +215,9 @@ TOKEN_CHARS_SET = frozenset(
 # =============================================================================
 # 工具函数
 # =============================================================================
+
...
+
 def parse_options_header(value: str | bytes | None) -> tuple[bytes, dict[bytes, bytes]]:
     """
     Parses a Content-Type header into a value in the following format: (content_type, {parameters}).
@@ -230,8 +233,10 @@ def parse_options_header(value: str | bytes | None) -> tuple[bytes, dict[bytes, bytes]]:
     # If we are passed bytes, we assume that it conforms to WSGI, encoding in latin-1.
     if isinstance(value, bytes):  # pragma: no cover
         value = value.decode("latin-1")
...
+
+    # 【优化】使用健壮的参数解析器
+    ctype, options = _robust_parse_options(value)
...
+
+
+def _robust_parse_options(value: str) -> tuple[str, dict[str, str]]:
+    【算法实现】健壮的 HTTP Header 参数解析
+
+    【功能说明】
+    使用状态机模式解析 HTTP Header 参数，正确处理：
+    1. 引号内的分号（不被当作参数分隔符）
+    2. 转义引号（\" → "）
+    3. 引号外的前导/尾随空格
+
+    【参数】
+    value: HTTP Header 值字符串
+
+    【返回值】
+    (content_type: str, options: dict[str, str]) 元组
+    """
+    # 初始化结果
...
+        BEFORE_KEY = 0   # 等待参数键名
+        IN_KEY = 1       # 解析键名
+        BEFORE_VALUE = 2 # 等待参数值
+        IN_VALUE_QUOTED = 3   # 解析带引号的值
+        IN_VALUE_UNQUOTED = 4 # 解析不带引号的值
+
+    i = 0
...
+            else:
+                current_value = ch
+                state = State.IN_VALUE_UNQUOTED
+                i += 1
+
+        elif state == State.IN_VALUE_QUOTED:
+            # 处理转义字符
+            if ch == "\\" and i + 1 < length:
+                next_ch = param_string[i + 1]
+                current_value += next_ch
+                i += 2
+            elif ch == '"':
+                # 保存结果
+                options[current_key.strip().lower()] = current_value
+                current_key = ""
+                current_value = ""
+                state = State.BEFORE_KEY
+                i += 1
...
+                i += 1
+
+        elif state == State.IN_VALUE_UNQUOTED:
+            if ch == ";":
+                options[current_key.strip().lower()] = current_value.strip()
+                current_key = ""
+                current_value = ""
+                state = State.BEFORE_KEY
+                i += 1
...
+
+    # 处理最后一个参数（如果没有以 ; 结尾）
+    if current_key and state != State.IN_VALUE_QUOTED:
+        options[current_key.strip().lower()] = current_value.strip()
+
+    # 处理 IE6 文件名路径 bug
...
+    if filename:
+        # 检查是否是 Windows 完整路径
+        if (len(filename) >= 3 and filename[1:3] == ":\\") or filename.startswith("\\\\"):
+            # 提取路径的最后一部分作为文件名
